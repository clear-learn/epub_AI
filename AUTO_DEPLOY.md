...
파일을 수정하면 자동으로 커밋/푸시/배포되는 시스템입니다.
...
### 2. 코드 수정
- 아무 파일이나 수정하세요
- 5초마다 변경사항을 자동으로 감지합니다
- 자동으로 commit → push → EC2 배포
...
```
파일 수정 → 5초 대기 → 자동 감지 → git add . → git commit → git push → GitHub Actions → EC2 배포
```

**배포 시간:** 파일 수정 후 약 1-2분
