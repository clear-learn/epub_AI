# 🤖 자동 배포 가이드 (로컬 → EC2 직접 배포)

파일을 수정하면 자동으로 EC2로 직접 배포되는 시스템입니다.
...
```
파일 수정 → 3초 대기 → 자동 감지 → rsync로 EC2 동기화 → pip install → 서비스 재시작
```

**배포 시간:** 파일 수정 후 약 10-20초 (GitHub Actions보다 훨씬 빠름!)
